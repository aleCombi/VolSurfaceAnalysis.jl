...
| **No-lookahead at the type level** | Both `current_policy` and `decide` take `TimeCutModelDataSource`, not `ModelDataSource`. Through exported accessors, neither an agent's refit logic nor a policy's decision logic can accidentally reach future observations. Master enforced this with a runtime wrapper passed in via an argument; the rebuild lifts it into the function signature. |
