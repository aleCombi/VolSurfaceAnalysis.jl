...
engine holds no state beyond the ledger and the book it folds.
...
1. **Lifecycle** (slice 3): expiries due at or before `t` are booked so
   ...
   empty until then.
2. **Decide.** The cut at `t`, the current policy, and the book: the
   engine's own fold of the ledger, equal to `book_as_known(L, known_to)`
   for every order record the tick produces. A policy reads it and must
   not mutate it.
...
the ledger, its counters, its order journal and the book as they were
before the order.
