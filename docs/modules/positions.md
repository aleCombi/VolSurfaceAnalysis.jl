...
pnl(position, settlement_spot)            :: Float64
pnl(positions, settlement_spot)           :: Float64
```
...
- **`pnl`** -- realized PnL at expiry: `payoff(trade, spot) - entry_cost(pos)`.
  Named as a noun (what the function returns), not a verb -- the function
  is pure, nothing is mutated. The vector overload sums across legs
  (condor, strangle, ...) at one settlement spot.
...
- Mutation of any kind. Closing a position is modelled by computing its
  `pnl` against the settlement spot, not by mutating fields.
...
| **`payoff` on `Trade`, `pnl` on `Position`** | Scenario analysis (breakevens, payoff diagrams) needs only the contract and a hypothetical spot; nothing about what was paid. Realized PnL needs the entry snapshot. Splitting cleanly separates "structure of the contract" from "what happened at fill." |
| **`pnl` is a noun, not `settle`** | `settle` reads like a mutation (settle the account, close the position). The function is pure -- it returns a number, changes nothing. Naming it after what it returns (`pnl`) keeps the API readable and leaves room for a future `value(position, surface)` for mark-to-market without overloading any verb. |
| **Absolute USD prices, not fraction-of-spot** | Equity-native. `entry_cost = entry_price * direction * quantity`, no multiplication by `entry_spot`. Master's fractional convention was Deribit legacy and is dropped. `entry_spot` is kept as a diagnostic, not a multiplier. |
...
| **No mutation, no portfolio type** | A multi-leg structure is a `Vector{Position}`. The vector `pnl` overload composes legs without introducing a `Portfolio` wrapper. Strategies / engines that want grouping can add their own type without touching this layer. |
...
- **Mark-to-market.** A `value(position, surface, settlement_curve)`
  ...
  mutation required.
- **Multi-asset portfolio aggregation.** Today the vector `pnl` overload
  assumes one settlement spot for all legs. A future overload taking
  a `Dict{Underlying, Float64}` (or a settlement source) covers
