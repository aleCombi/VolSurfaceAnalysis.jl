using VolSurfaceAnalysis: parse_massive_ticker, et_to_utc, Call, Put

@testset "parse_massive_ticker: SPY call standard" begin
    u, expiry, otype, strike = parse_massive_ticker("O:SPY240129C00406000")
    @test u == "SPY"
    @test otype == Call
    @test strike == 406.0
...
@testset "parse_massive_ticker: DST boundary" begin
    _, expiry_winter, _, _ = parse_massive_ticker("O:SPY240301C00400000")
    @test expiry_winter == DateTime(2024, 3, 1, 21, 0)

    _, expiry_summer, _, _ = parse_massive_ticker("O:SPY240715C00400000")
    @test expiry_summer == DateTime(2024, 7, 15, 20, 0)
end

@testset "parse_massive_ticker: put + malformed" begin
    _, _, otype, strike = parse_massive_ticker("O:QQQ240115P00350500")
    @test otype == Put
    @test strike == 350.5

    @test_throws ArgumentError parse_massive_ticker("O:SPY24")
    @test_throws ArgumentError parse_massive_ticker("not_a_ticker")
end
