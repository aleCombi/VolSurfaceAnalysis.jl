# scripts/spy_sizing_logsig_condor.jl
#
# Same as `spy_sizing_logsig.jl` but trade an iron CONDOR instead of a naked
# strangle. Shorts: 20Δp / 5Δc (matches strangle). Long wings: fixed $WING_WIDTH
# OTM from each short (caps max-loss per contract).
#
# Goal: see whether the binary skip-filter still adds value once tail risk is
# already structurally capped by the wings.
...
PUT_DELTA           = 0.20
CALL_DELTA          = 0.05
PUT_WING_WIDTH      = 10.0   # wider put wing — embraces put-skew protection
CALL_WING_WIDTH     = 3.0    # tight call wing — minimal premium given low call-side disasters
...
run_dir = joinpath(@__DIR__, "runs", "spy_sizing_logsig_condor_asym_$run_ts")
mkpath(run_dir)
...
each_entry(source, EXPIRY_INTERVAL, sched; clear_cache=true) do ctx, settlement
    ...
    # Short legs by delta (same as strangle setup)
    ...
    # Long wings: nearest available strike to (short ± WING_WIDTH), must be OTM relative to short.
    target_lp = sp_K - PUT_WING_WIDTH
    target_lc = sc_K + CALL_WING_WIDTH
    otm_put_recs  = filter(r -> r.strike <  sp_K, dctx.put_recs)
    otm_call_recs = filter(r -> r.strike >  sc_K, dctx.call_recs)
    (isempty(otm_put_recs) || isempty(otm_call_recs)) && (global n_skip_strk += 1; return)
    ...
    length(condor_pos) == 4 || (global n_skip_strk += 1; return)
    pnl_usd = settle(condor_pos, Float64(settlement)) * spot
...
function build_folds(d, train_days, test_days, step)
    ...
    while ts_start <= last_d
        ...
        if sum(tr_mask) >= 200 && sum(te_mask) >= 20
            push!(folds, (
