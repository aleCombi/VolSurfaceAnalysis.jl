...
ENTRY_TIMES         = [Time(11, 0), Time(13, 0), Time(15, 0)]   # 3 entries/day
EXPIRY_INTERVAL     = Hour(5)            # targets ~4pm — closest available expiry = same-day 4pm for all 3 entries
SPREAD_LAMBDA       = 0.7
RATE                = 0.045
DIV_YIELD           = 0.013
...
FEATURES            = Feature[
    # Rolling 2h lookback works for all 3 entry times (11:00, 13:00, 15:00)
    SpotMinuteLogSig(lookback_hours=2, depth=3, min_points=60),
]
...
run_dir = joinpath(@__DIR__, "runs", "spy_sizing_condor_3x_intraday_$run_ts")
mkpath(run_dir)
