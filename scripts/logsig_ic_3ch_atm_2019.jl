...
# =============================================================================
# Custom feature: (time, log_return, ATM_IV) 3-channel logsig.
# Combines SpotMinuteLogSig's time-augmented spot path with IntradayLogSig's
# ATM-IV channel. Reuses the production `_bulk_atm_iv` + `BarCache`.
# =============================================================================
...
FEATURES            = Feature[
    TimeSpotIVLogSig(depth=3, rate=RATE, div_yield=DIV_YIELD, min_points=30),
]
...
run_dir = joinpath(@__DIR__, "runs", "logsig_ic_3ch_atm_2019_$run_ts")
mkpath(run_dir)
...
function _evict_caches!(src::ParquetDataSource, feats::Vector{<:Feature})
    ...
    for f in feats
        if f isa IntradayLogSig || f isa TimeSpotIVLogSig
            empty!(f.cache.data)
        end
    end
    nothing
